...
class Parser(IParser):
    ...
    def parse(self, path: str) -> List[IEntry]:
        print(f"Начинаем парсинг {path}")
        start_time = time.time()
        logs, parsed_ua_list = self._parse_logs(path)
        out = [Entry(*log).set_ua(UserAgents(*ua)) for log, ua in zip(logs, parsed_ua_list)]
        print(f"[✓] Парсинг '{path}' завершён за {time.time() - start_time:.2f} секунд.")
        return out
