...
def process_log_file(file_path):
    ...
    logging.info(f"📝 Parsed {len(logs)} logs in {time.time() - start_time:.2f} sec")
    ...
    buffer.seek(0)
    logging.info(f"📝 csv write logs in {time.time() - start_time:.2f} sec")
    ...
    conn.commit()
    logging.info(f"✅ Data loaded to staging_logs in {time.time() - start_time:.2f} sec")

    # Добавление индексов
    execute_sql_file(cursor, "idx_staging_logs.sql")
    logging.info(f"staging_logs: индексы добавлены")
    # Перенос данных в fact_logs
    execute_sql_file(cursor, "transfer_staging_to_fact.sql")
    logging.info(f"staging_logs: fact_logs обновлен")
    # Удаление staging таблицы
    cursor.execute(f"DROP TABLE IF EXISTS staging_logs;")
    logging.info(f"🧹 staging_logs удалена")
